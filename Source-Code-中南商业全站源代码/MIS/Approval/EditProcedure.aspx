<%@ page language="C#" autoeventwireup="true" inherits="MIS_Approval_EditProcedure, App_Web_qfkgrtvd" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE html>
<html>
<head runat="server">
...
document.getElementById("CommentDiv").style.top = (h - 124) + "px";
...
//添加列内容
newPesonTD.innerHTML = "<asp:Lable id='lblPeson" + rowID + "'></asp:Lable><a href=\"#\" onclick=\"PopupDiv2('div_share','lblChecked')\">[选择]</a><input name='txtPeson" + rowID + "' id='txtPeson" + rowID + "' type='hidden' size='6' value='" + rowID + "' />";
//添加列:抄送给
var newPesonTD = newTR.insertCell(3);
//添加列内容
newPesonTD.innerHTML = "<asp:Lable id='lblSend" + rowID + "'></asp:Lable><a href=\"#\" onclick=\"PopupDiv2('div_share','lblChecked')\">[选择]</a><input name='txtSend" + rowID + "' id='txtSend" + rowID + "' type='hidden' size='6' value='" + rowID + "' />";
...
<div id="Procedure">
<div class="PType">流程类型:<asp:DropDownList ID="DrpType" runat="server"></asp:DropDownList><a href="#" onclick="putid('CommentDiv')">添加类型</a><div id="divcon"></div></div>
<div class="PName">
    <span>*</span>流程名称:
...
<td class="ThPeson"> 
   <asp:Label ID="lblApprover" runat="server" Text='<%#CheckUserExit(Convert.ToInt32(Eval("UserID")))%>'></asp:Label> <a href="javascript:void(0)" onclick="PopupDivs2('div_share','lblChecked','<%#Eval("ID")%>')";>[选择]</a></td>
<td class="ThSend"><asp:Label ID="lblSend" runat="server" Text='<%#Eval("Send")%>'></asp:Label><a href="javascript:void(0)" onclick="PopupDiv2('div_share','lblChecked','<%#Eval("ID")%>')";>[选择]</a></td>
<td class="ThOperation"><asp:LinkButton ID="linkBtnDelete" runat="server" CommandName="Delete" CommandArgument='<%#Eval("ID")%>' OnClientClick="return confirm('确定要删除该条记录么？')"  Text="[删除]" Enabled='<%#Convert.ToInt32(Eval("Level"))==1?false:true%>'></asp:LinkButton></td>
</tr>
</ItemTemplate>
</asp:Repeater>
</table>
...
<iframe src="/Mis/SelUser.aspx?OpenerText=TxtApprover" width="420" height="120" scrolling="no" frameborder="0"></iframe>
<div class="p_bottom">
<asp:Button ID="SubBtns" runat="server" Text="确定" CssClass="i_bottom" OnClick="SubBtns_Click" />
</div>
</div>
