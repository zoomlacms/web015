<%@ page language="C#" autoeventwireup="true" validaterequest="false" inherits="ZoomLa.WebSite.Manage.Content.EditContent, App_Web_hy2ojcfu" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<table style="width: 100%; margin: 0 auto;" cellpadding="2" cellspacing="1" class="border">
		...
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			...
			<td class="tdbg">
				<asp:TextBox ID="txtTitle" CssClass="l_input" onblur="getstrKeys()" onkeyup="isgoEmpty('txtTitle','span_txtTitle');query('txtTitle','PYtitle')"
					runat="server" Text='' MaxLength="255" Style="width: 41%; background-image: url('/ZoomLa.WebSite/Images/bg.gif');"></asp:TextBox>
	...
	<tbody id="Tabs1" style="display: none">
		...
		<tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
			...
			<td class="bqright">
				&nbsp;<input id="Radio1" type="radio" name="date" runat="server" value="1" onclick="datecontrol(true)" />保持为原来的时间
				<br />
				&nbsp;<input id="Radio2" type="radio" name="date" runat="server" value="2" onclick="datecontrol(false)" />自定义&nbsp;&nbsp;
	...
	<tr class="tdbgbottom">
		<td colspan="2">
			<asp:Button ID="EBtnSubmit" CssClass="C_input" Text="保存项目" runat="server" OnClick="EBtnSubmit_Click"  />&nbsp;
			<asp:Button ID="Button1"    CssClass="C_input" Text="添加为新项目" runat="server" OnClick="Button1_Click" />&nbsp;
			<asp:Button ID="BtnBack"    CssClass="C_input" Text="返　回"  runat="server" OnClick="BtnBack_Click" UseSubmitBehavior="False" CausesValidation="False" />
            <asp:Button ID="BtnDoc" CssClass="C_input" runat="server"  OnClientClick="return generate('docTitle','docParamValue');" Text="重新生成文档"  Visible="false"/>
...
<script type="text/javascript">
	function change(id) {
		...
		var srcva;
		if (vas[1] == "饼状图") {
			srcva =<%=PageAction.customPath2%>+"Charts/pie-basic.aspx?Cid=" + cid;
		} else if (vas[1] == "线状图") {
		    srcva =<%=PageAction.customPath2%> +"Charts/line.aspx?Cid=" + cid;
		} else {
		    srcva = <%=PageAction.customPath2%> +"Charts/colum.aspx?Cid=" + cid;
		}
	...
	function Uifra(width, height, title, type, id) {
		...
		var cid = vas[0];
		var srcva;
		srcva = "<%=PageAction.customPath2%>Charts/Show.aspx?Did=" + cid;
	...
	function Uchart(id) {
		var hid = id.substr(5);
		window.open('<%=PageAction.customPath2%>Flex/CAddChart.aspx?cdid=' + id + '&ChartID=' + $("#" + hid).val(), 'newWin', 'modal=yes,width=800,height=500,resizable=yes,scrollbars=yes');
	}
