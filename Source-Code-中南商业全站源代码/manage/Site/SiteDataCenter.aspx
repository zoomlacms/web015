<%@ page language="C#" autoeventwireup="true" inherits="manage_Site_SiteDataCenter, App_Web_1zxlesnb" masterpagefile="~/manage/Site/SiteMaster.master" title="数据中心" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<script src="../../JS/jquery.js"></script>
      ...
      * {
      ...
      .leftDiv {
          ...
          BACKGROUND: url(/res/static/images/aside-right-shadow.jpg?@=-1) #f0f0f0 repeat-y right top;
          /*POSITION: absolute;*/
          float:left;
          PADDING-TOP: 10px;
          LEFT: 0px;
      ...
      .rightDiv-Head {
          ...
          BACKGROUND: url(/res/static/images/outer_bn2.gif?t=201311212929?@=-1) #f7f7f7 repeat-x left -53px;
          POSITION: relative;
          Z-INDEX: 2;
      }
      .rightDiv {
...
<div id="m_site"><p> 站群中心 >> 数据中心</p></div>
  ...
  <asp:Repeater runat="server" ID="siteRepeater">
      ...
      <tr>
         <td><input type="checkbox" name="siteChk" value='<%# GetDomain(Eval("Domain").ToString(),Eval("SitePort").ToString()) %>' style="float:left;"/>
             <%#Eval("SiteName") %></td><td> <%#Eval("Domain") %></td>
      </tr>
      </ItemTemplate>
  </asp:Repeater>
</table>
<asp:TextBox runat="server" ID="urlText" Width="90%" style="margin-top:30px;" ></asp:TextBox>
<asp:Literal runat="server" ID="remind"></asp:Literal>
</asp:Panel>
</div>
    
<div id="rightDiv" class="rightDiv">
<!--主体头部-->
<div class="rightDiv-Head" style="height:40px;">
       <%-- <asp:TextBox runat="server" ID="siteUrl" Width="500px" TextMode="MultiLine"></asp:TextBox>
        <asp:Label runat="server" ID="siteLabel"></asp:Label> <br />--%>
    <div style="padding-top:8px;">
        <asp:Button runat="server" ID="beginGet" Text="开始获取" OnClick="beginGet_Click" CssClass="site_button"/>
        <asp:Button runat="server" ID="btn1" Text="加入数据库"  OnClick="btn1_Click" CssClass="site_button" OnClientClick="return checkData()"/>
        将原属节点 <asp:DropDownList runat="server" ID="mainDp1"></asp:DropDownList>
        改为<asp:DropDownList runat="server" ID="mainDp2"></asp:DropDownList>
        <input type="button" value="确定" onclick="changeNode()" class="site_button"/>
        <asp:Button runat="server" ID="logBtn" Text="查看日志" OnClick="logBtn_Click" CssClass="site_button"/>
